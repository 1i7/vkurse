...
<html>
	...
	for(int i=1; i<8 ; i++)
    {%>
     	 <tr bgcolor="#FF8455" border=10><td><a href="../day.jsp?group=<%= group %>&day=<%= i %>"><%= week[i] %></a></td>
		 <td>
		 <% 
		 {
		 int temp = 1440;
		 ...
		 %>
			...
			%>
				...
				<tr><td>
				<a href="edit_schedule.jsp?action=1&group=<%= group%>&id=<%= ((Schedule)lst[i].get(k)).getID() %>">Удалить</a>
				</td></tr>
				</table>  
			</div>
		    </td>
